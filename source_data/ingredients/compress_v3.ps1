        ...
        try {
            ...
            for ($i = 0; $i -lt $maxAttempts; $i++) {
                ...
                if ($tempFileSize -le $targetSizeKB * 1024) {
                    ...
                    Write-Host "    SUCCESS: Quality=$quality%, New=$newSizeKB KB, Saved=$savedKB KB ($savedPercent% reduction)" -ForegroundColor Green
                    $success = $true
                    break
                } else {
...
Write-Host "Space saved: $savedTotalMB MB ($savedTotalPercent% reduction)" -ForegroundColor Cyan
Write-Host ""
