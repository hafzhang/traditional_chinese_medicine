...
# Configuration
$targetSizeKB = 200
$sourceDir = "C:\Users\Administrator\Documents\claude杩囨护钄彍鍥?
        ...
        try {
            ...
            for ($i = 0; $i -lt $maxAttempts; $i++) {
                ...
                if ($tempFileSize -le $targetSizeKB * 1024) {
                    ...
                    Write-Host "    SUCCESS: Quality=$quality%, New=$newSizeKB KB, Saved=$savedKB KB ($savedPercent%)" -ForegroundColor Green
                    $success = $true
                    break
                } else {
...
Write-Host "Space saved: $savedTotalMB MB ($savedTotalPercent)" -ForegroundColor Cyan
Write-Host ""
